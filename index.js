...
module.exports = function(bp) {
  ...
  bp.hear({ 'nlp.action': 'yahooWeatherForecast' }, (event, next) => {
    //Action performed when user asks for weather forcast
  ...
  bp.hear({ 'nlp.action': 'greet' }, (event, next) => {
    //Action performed for greeting user. For Eg. Hi/Hello    
    event.reply('#greeting')
  })

  bp.hear('GREETING.B1', (event, next) => {
    //If user selects "Current Weather" quick reply
  ...
  bp.hear('CONDITION.B1', (event, next) => {
    //If user selects "Current Weather" quick reply of condition message
    event.reply('#promptLocation')
  })
